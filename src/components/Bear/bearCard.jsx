        ...
        <AnimatePresence>
            {(animal === 'bear' || prevAnimal === 'bear') &&
                <motion.div className={style.bearCard} style={animal !== 'bear' && { position: 'absolute', top: 0, right: 0 }}
                    initial={load ? false : (animal === 'bear' ? { scaleX: 0.4, originX: 1 } : { scaleX: 1.02, originX: 1 })}
                    animate={load ? false : (animal === 'bear' ? { scaleX: 1, originX: 1 } : { scaleX: 0.423, originX: 1, opacity: 0 })}
                    transition={{ duration: 0.4 }} >

                    <motion.div className={style.bearCardCont}
                        initial={load ? false : (animal === 'bear' ? { scaleX: 0.5, scaleY: 0.5, x: 120, y: 45 } : { scaleX: 1, scaleY: 1, x: 0, y: 0 })}
                        animate={load ? false : (animal === 'bear' ? { scaleX: 1, scaleY: 1, x: 0, y: 0 } : { scaleX: 0.5, scaleY: 0.5, x: 120, y: 45 })}
                        transition={{ duration: 0.4 }}>
